...
TE_MODEL_RIBO_DATA = VENDOR_TE_MODEL / "data" / "ribo"
...
def _alias_patch(organism: str) -> str:
    """Return runtime alias setup for generated TE_model trial config."""
...
def _write_trial(trial_dir: Path, experiments: list[str], organism: str = "Homo sapiens") -> None:
    trial_dir.mkdir(parents=True, exist_ok=True)
    (trial_dir / "__init__.py").write_text("")
    alias_patch = _alias_patch(organism)
    (trial_dir / "config.py").write_text(
        ...
        f"     custom_experiment_list={experiments!r})\n"
    )


def _inject_ribo_input(study_dir: Path, study: str) -> Path:
    """Symlink snakescale output into TE_model's runtime data/ribo area."""
    src = study_dir.resolve()
    ribo_dir = src / "ribo"
    if not (ribo_dir / "all.ribo").is_file():
        raise FileNotFoundError(f"all.ribo missing under {ribo_dir}")

    dst = TE_MODEL_RIBO_DATA / study
    dst.parent.mkdir(parents=True, exist_ok=True)
    if dst.is_symlink():
        dst.unlink()
    elif dst.exists():
        raise FileExistsError(f"TE_model ribo input path is not a symlink: {dst}")
    dst.symlink_to(src)
    return dst
...
def main(argv: list[str] | None = None) -> int:
    ...
    ribo_link = _inject_ribo_input(args.study_dir, study)
    print(f"[run_downstream] linked {ribo_link} -> {ribo_link.resolve()}")
    trial_dir = TE_MODEL_TRIALS / study
    _write_trial(trial_dir, experiments, organism)

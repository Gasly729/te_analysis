...
def build_argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--mapping-output",
        type=Path,
        default=DEFAULT_RAW_ROOT / "experiment_run_mapping_local.tsv",
    )
    parser.add_argument(
        "--mapping-report-output",
        type=Path,
        default=DEFAULT_RAW_ROOT / "_experiment_run_mapping_report.md",
    )
...
def main() -> int:
    ...
    result = build_metadata_runs_outputs(
        ...
        mapping_report_path=args.mapping_report_output.expanduser().resolve(),
    ...
    print(f"mapping_report_path: {result.mapping_report_path}")
    print(f"runs_path: {result.runs_path}")
    print(f"unresolved_path: {result.unresolved_path}")
    print(f"report_path: {result.report_path}")
    print(f"mapping_rows: {len(result.mapping.discovery.mapping_frame)}")
